...
如果我们的所有查询语句都是可选条件，那么当没有条件存在时可能会导致语句解析错误，此时我们可以利用 where 标签：
...
sql 这个元素可以被用来定义可重用的 SQL 代码段，这些 SQL 代码可以被包含在其他语句中。它可以（在加载的时候）被静态地设置参数。在不同的包含语句中可以设置不同的值到参数占位符上。比如：
