...
---
title: 2020-heibaiying-计算机网络
linktitle: 2020-heibaiying-计算机网络
type: book
commentable: true
---
...
如上图所示，当 M<sub>3</sub> 丢失时，之后发送 M<sub>4</sub> ， M<sub>5</sub> ， M<sub>6</sub> 时收到的都是对于 M<sub>2</sub> 的重复确认，此时发送方就可以知道 M<sub>3</sub> 已经丢失，需要立即进行重传。由于此时只是个别报文出现了丢失，而不是网络拥塞，所以执行**快恢复**：发送方调整 ssthresh = cwnd / 2，并设置 cwnd = ssthresh = 8 （图中点 5），并开始执行拥塞避免算法。
